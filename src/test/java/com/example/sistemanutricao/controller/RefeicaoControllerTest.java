...
@DisplayName("RefeicaoController - Unitário com MockMvc")
class RefeicaoControllerTest {
    ...
    void setup() {
        ...
        RefeicaoController controller = new RefeicaoController(refeicaoService, refeicaoQueryService, fichaTecnicaService, paginacaoViewSupport);
    ...
    @Test
    @DisplayName("Deve buscar refeições ativas por padrão")
    void deveBuscarAtivasPadrao() throws Exception {
        Page<RefeicaoResponseDTO> page = new PageImpl<>(List.of());
        
        when(refeicaoQueryService.buscarPorStatus(eq(Status.ATIVA), eq(usuarioMock.getUsuario()), any(Pageable.class)))
                .thenReturn(page);
        ...
        mockMvc.perform(get("/refeicao"))
                ...
                .andExpect(model().attributeExists("refeicoes"))
                .andExpect(view().name("pages/refeicoes/List"));

        verify(refeicaoQueryService).buscarPorStatus(eq(Status.ATIVA), eq(usuarioMock.getUsuario()), any(Pageable.class));
    }
